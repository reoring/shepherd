...
- Keep the Sheperd public contract stable: Pi `/sheperd query <file-or-directory> [--contract <contract.json>] -- <question>` and `/sheperd check <directory> --contract <contract.json>`; shell `sheperd query <file-or-directory> --question <text> [--contract <contract.json>] [--model provider/model] [--isolation subprocess|docker] [--json]` and `sheperd check <directory> --contract <contract.json> [--isolation subprocess|docker] [--json]`.
- Treat RLM as the recursive runtime technique underneath Sheperd, not as a command or compatibility surface.
